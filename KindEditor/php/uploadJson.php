...
if (empty($_FILES) === false) {
	...
	//检查扩展名
	if (in_array($file_ext, $ext_arr[$dir_name]) === false) alert('上传文件扩展名是不允许的扩展名。只允许' . implode(',', $ext_arr[$dir_name]) . '格式!');
	//创建文件夹
	if ($dir_name !== '') {
		$save_path .= $dir_name . '/';
		$save_url .= $dir_name . '/';
...
function alert($msg) { die(json_encode(['error' => 1, 'message' => $msg])); }